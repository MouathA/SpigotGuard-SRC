...
public class PacketDecoder_1_16_R2 extends PacketDecoder
{
    ...
    private LimitablePacket hasReachedLimit(final String s) {
        if (!PacketDecoder_1_16_R2.LIMITATION.isLimitable(s)) {
            return null;
        }
        final LimitablePacket limit = PacketDecoder_1_16_R2.LIMITATION.getLimit(s);
        ...
        if (o.getClass().getSimpleName().equals("PacketPlayInWindowClick")) {
            ...
            try {
                ...
                if (openInventory != null) {
                    ...
                    if (b >= n && PacketDecoder_1_16_R2.LIMITATION != null) {
            ...
            final double n3 = packetPlayInPosition.a(0.0f);
            final double n4 = packetPlayInPosition.b(0.0f);
            final double c = packetPlayInPosition.c(0.0);
            if ((n3 >= Double.MAX_VALUE || n4 > Double.MAX_VALUE || c >= Double.MAX_VALUE) && !Settings.IMP.POSITION_CHECKS.ALLOW_INVALID_MOVEMENT_V2) {
            ...
            if (n3 >= 2.147483647E9 || n4 > 2.147483647E9 || (c >= 2.147483647E9 && !Settings.IMP.POSITION_CHECKS.ALLOW_INVALID_MOVEMENT_V1)) {
            ...
            if (Settings.IMP.POSITION_CHECKS.CHECK_CHUNK_CRASHER) {
                final double n5 = Math.max(player.getLocation().getX(), n3) - Math.min(player.getLocation().getX(), n3);
                if (n5 >= 10.0 && n5 % 1.0 == 0.0) {
                    SpigotGuardLogger.log(Level.INFO, String.valueOf(new StringBuilder().append("Received invalid location packet from: ").append(player.getName()).append(" (").append(player.getLocation().getX()).append(",").append(player.getLocation().getY()).append(",").append(player.getLocation().getZ()).append(" ||| Packet: ").append(n3).append(",").append(n4).append(",").append(c).append("), difference: ").append(n5)), new Object[0]);
                    if (this.hasReachedLimit("PacketPlayInPosition_Invalid_v1") != null) {
                        return new ExploitDetails(this.user, simpleName, String.valueOf(new StringBuilder().append("Chunk crasher (v1, invalid x coords, diff: ").append(n5).append(")")), false, false, "set position-checks.check-chunk-crasher to false in settings.yml or increase PacketPlayInPosition_Invalid_v1.limit in config.yml");
                    }
                }
                final double n6 = Math.max(player.getLocation().getZ(), c) - Math.min(player.getLocation().getZ(), c);
                if (n6 >= 10.0 && n6 % 1.0 == 0.0) {
                    SpigotGuardLogger.log(Level.INFO, String.valueOf(new StringBuilder().append("Received invalid location packet from: ").append(player.getName()).append(" (").append(player.getLocation().getX()).append(",").append(player.getLocation().getY()).append(",").append(player.getLocation().getZ()).append(" ||| Packet: ").append(n3).append(",").append(n4).append(",").append(c).append("), difference: ").append(n6)), new Object[0]);
                    if (this.hasReachedLimit("PacketPlayInPosition_Invalid_v1") != null) {
                        return new ExploitDetails(this.user, simpleName, String.valueOf(new StringBuilder().append("Chunk crasher (v1, invalid z coords, diff: ").append(n6).append(")")), false, false, "set position-checks.check-chunk-crasher to false in settings.yml or increase PacketPlayInPosition_Invalid_v1.limit in config.yml");
                    }
                }
                if (Settings.IMP.POSITION_CHECKS.CHECK_FLY_CRASHER && n4 == player.getLocation().getY() + 0.1) {
                    final double n7 = Math.max(player.getLocation().getY(), n4) - Math.min(player.getLocation().getY(), n4);
                    SpigotGuardLogger.log(Level.INFO, String.valueOf(new StringBuilder().append("Received invalid Y location packet from: ").append(player.getName()).append(" (").append(player.getLocation().getX()).append(",").append(player.getLocation().getY()).append(",").append(player.getLocation().getZ()).append(" ||| Packet: ").append(n3).append(",").append(n4).append(",").append(c).append("), difference: ").append(n7)), new Object[0]);
                    if (this.hasReachedLimit("PacketPlayInPosition_Invalid_v2") != null) {
                        return new ExploitDetails(this.user, simpleName, String.valueOf(new StringBuilder().append("Fly crasher (v1, invalid y coords, diff: ").append(n7).append(")")), false, false, "set position-checks.check-fly-crasher to false in settings.yml or increase PacketPlayInPosition_Invalid_v2.limit in config.yml");
                    }
                }
            }
        }
            ...
            if (Settings.IMP.POSITION_CHECKS.CHECK_YAW_CRASHER && (a2 < -40000.0f || b3 < -40000.0f || a2 > 40000.0f || b3 > 40000.0f || a2 == 9.223372E18 || b3 == 9.223372E18 || a2 == 9.223372E18f || b3 == 9.223372E18f)) {
    ...
    public PacketDecoder_1_16_R2(final PacketInjector packetInjector, final User user) {
                ...
                if (compound2.hasKey("textures")) {
                    ...
                    for (int l = 0; l < list2.size(); ++l) {
                        ...
                        if (compound3.hasKey("Value")) {
                            ...
                            final Matcher matcher = PacketDecoder_1_16_R2.URL_MATCHER.matcher(lowerCase2);
                            while (matcher.find()) {
