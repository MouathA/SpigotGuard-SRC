package xyz.yooniks.spigotguard.network.v1_17;
...
public class PacketDecoder_1_17_R1 extends PacketDecoder
{
    ...
    public ExploitDetails failure(final Object o) {
        ...
        if (o.getClass().getSimpleName().equals("PacketPlayInWindowClick")) {
            ...
            try {
                ...
                if (e == null) {
                    try {
                        final Field declaredField = PacketPlayInWindowClick.class.getDeclaredField("e");
                ...
                if (openInventory != null) {
                    ...
                    if (b >= n && PacketDecoder_1_17_R1.LIMITATION != null) {
                    ...
                    if (e != null) {
                        ...
                        if (bukkitCopy.getType() == Material.CHEST || bukkitCopy.getType() == Material.HOPPER) {
                            if (bukkitCopy.hasItemMeta() && bukkitCopy.getItemMeta().toString().getBytes().length > 262144) {
                                return new ExploitDetails(this.user, simpleName, "too big chest data", false, false);
                            }
                            if (e.getTag() != null && e.getTag().toString().contains("www.wurstclient.net")) {
                                return new ExploitDetails(this.user, simpleName, "wurst chest lagger", false, false);
                            }
                        }
                    }
                }
            ...
            final String key = packetPlayInCustomPayload.c.getKey();
            final PacketDataSerializer d = packetPlayInCustomPayload.d;
            if (d.capacity() > Settings.IMP.PAYLOAD_SETTINGS.MAX_CAPACITY) {
            ...
            Label_1356: {
                if (key.equals("MC|BEdit") || key.equals("MC|BSign") || key.equals("minecraft:bedit") || key.equals("minecraft:bsign")) {
                    if (Settings.IMP.DEBUG) {
                        SpigotGuardLogger.log(Level.INFO, "Received {0} from {1}", simpleName, player.getName());
                    }
                    try {
                        final ItemStack o2 = d.o();
                        if (o2 != null) {
                            ...
                            final ExploitDetails checkNbtTags3 = this.checkNbtTags(simpleName, o2);
                ...
                if (key.equals("REGISTER") || Integer.valueOf(1321107516).equals(key.toUpperCase().hashCode()) || key.toLowerCase().contains("fml")) {
                    ByteBuf copy = null;
                    try {
                        copy = d.copy();
                ...
                else if (key.equals("MC|ItemName") && player.getInventory() != null && player.getOpenInventory().getType() != InventoryType.ANVIL && Settings.IMP.BLOCK_ITEMNAME_WHEN_NO_ANVIL) {
            ...
            final double a = packetPlayInPosition.a(0.0);
            final double b2 = packetPlayInPosition.b(0.0);
            final double c = packetPlayInPosition.c(0.0);
            if ((a >= Double.MAX_VALUE || b2 > Double.MAX_VALUE || c >= Double.MAX_VALUE) && !Settings.IMP.POSITION_CHECKS.ALLOW_INVALID_MOVEMENT_V2) {
            ...
            if (a >= 2.147483647E9 || b2 > 2.147483647E9 || (c >= 2.147483647E9 && !Settings.IMP.POSITION_CHECKS.ALLOW_INVALID_MOVEMENT_V1)) {
            ...
            if (Settings.IMP.POSITION_CHECKS.CHECK_CHUNK_CRASHER) {
                final double n2 = Math.max(player.getLocation().getX(), a) - Math.min(player.getLocation().getX(), a);
                if (n2 >= 10.0 && n2 % 1.0 == 0.0) {
                    SpigotGuardLogger.log(Level.INFO, String.valueOf(new StringBuilder().append("Received invalid location packet from: ").append(player.getName()).append(" (").append(player.getLocation().getX()).append(",").append(player.getLocation().getY()).append(",").append(player.getLocation().getZ()).append(" ||| Packet: ").append(a).append(",").append(b2).append(",").append(c).append("), difference: ").append(n2)), new Object[0]);
                    if (n2 > 1000.0) {
                        return new ExploitDetails(this.user, simpleName, String.valueOf(new StringBuilder().append("Chunk crasher (v2, invalid x coords, diff: ").append(n2).append(")")), false, false, "set position-checks.check-chunk-crasher to false in settings.yml");
                    }
                    if (this.hasReachedLimit("PacketPlayInPosition_Invalid_v1") != null) {
                        return new ExploitDetails(this.user, simpleName, String.valueOf(new StringBuilder().append("Chunk crasher (v1, invalid x coords, diff: ").append(n2).append(")")), false, false, "set position-checks.check-chunk-crasher to false in settings.yml or increase PacketPlayInPosition_Invalid_v1.limit in config.yml");
                    }
                }
                final double n3 = Math.max(player.getLocation().getZ(), c) - Math.min(player.getLocation().getZ(), c);
                if (n3 >= 10.0 && n3 % 1.0 == 0.0) {
                    SpigotGuardLogger.log(Level.INFO, String.valueOf(new StringBuilder().append("Received invalid location packet from: ").append(player.getName()).append(" (").append(player.getLocation().getX()).append(",").append(player.getLocation().getY()).append(",").append(player.getLocation().getZ()).append(" ||| Packet: ").append(a).append(",").append(b2).append(",").append(c).append("), difference: ").append(n3)), new Object[0]);
                    if (n3 > 1000.0) {
                        return new ExploitDetails(this.user, simpleName, String.valueOf(new StringBuilder().append("Chunk crasher (v2, invalid z coords, diff: ").append(n3).append(")")), false, false, "set position-checks.check-chunk-crasher to false in settings.yml");
                    }
                    if (this.hasReachedLimit("PacketPlayInPosition_Invalid_v1") != null) {
                        return new ExploitDetails(this.user, simpleName, String.valueOf(new StringBuilder().append("Chunk crasher (v1, invalid z coords, diff: ").append(n3).append(")")), false, false, "set position-checks.check-chunk-crasher to false in settings.yml or increase PacketPlayInPosition_Invalid_v1.limit in config.yml");
                    }
                }
            }
            if (Settings.IMP.POSITION_CHECKS.CHECK_FLY_CRASHER && b2 == player.getLocation().getY() + 0.1) {
                final double n4 = Math.max(player.getLocation().getY(), b2) - Math.min(player.getLocation().getY(), b2);
                ...
                if (this.hasReachedLimit("PacketPlayInPosition_Invalid_v2") != null) {
                    return new ExploitDetails(this.user, simpleName, String.valueOf(new StringBuilder().append("Fly crasher (v1, invalid y coords, diff: ").append(n4).append(")")), false, false, "set position-checks.check-fly-crasher to false in settings.yml or increase PacketPlayInPosition_Invalid_v2.limit in config.yml");
                }
            }
            if (Settings.IMP.POSITION_CHECKS.CHECK_YAW_CRASHER) {
                final float a2 = packetPlayInPosition.a(0.0f);
                final float b3 = packetPlayInPosition.b(0.0f);
                if (a2 < -80000.0f || b3 < -80000.0f || a2 > 80000.0f || b3 > 80000.0f || a2 == 9.223372E18 || b3 == 9.223372E18 || a2 == 9.223372E18f || b3 == 9.223372E18f) {
                    return new ExploitDetails(this.user, simpleName, String.valueOf(new StringBuilder().append("Invalid yaw/pitch v1 (yaw: ").append(a2).append(", pitch: ").append(b3).append(")")), false, false, "set position-checks.check-yaw-crasher to false in settings.yml report to staff on mc-protection.eu/discord");
                }
            }
        }
        else if (o instanceof PacketPlayInFlying.PacketPlayInPositionLook) {
            final PacketPlayInFlying.PacketPlayInPositionLook packetPlayInPositionLook = (PacketPlayInFlying.PacketPlayInPositionLook)o;
            if (Settings.IMP.POSITION_CHECKS.CHECK_YAW_CRASHER) {
                final float a3 = packetPlayInPositionLook.a(0.0f);
                final float b4 = packetPlayInPositionLook.b(0.0f);
                if (a3 < -80000.0f || b4 < -80000.0f || a3 > 80000.0f || b4 > 80000.0f || a3 == 9.223372E18 || b4 == 9.223372E18 || a3 == 9.223372E18f || b4 == 9.223372E18f) {
                    return new ExploitDetails(this.user, simpleName, String.valueOf(new StringBuilder().append("Invalid yaw/pitch v2 (yaw: ").append(a3).append(", pitch: ").append(b4).append(")")), false, false, "set position-checks.check-yaw-crasher to false in settings.yml report to staff on mc-protection.eu/discord");
                }
            }
        }
        else if (o instanceof PacketPlayInFlying) {
            final PacketPlayInFlying packetPlayInFlying = (PacketPlayInFlying)o;
            final double a4 = packetPlayInFlying.a(0.0);
            final double b5 = packetPlayInFlying.b(0.0);
            final double c2 = packetPlayInFlying.c(0.0);
            if ((a4 >= Double.MAX_VALUE || b5 > Double.MAX_VALUE || c2 >= Double.MAX_VALUE) && !Settings.IMP.POSITION_CHECKS.ALLOW_INVALID_MOVEMENT_V2) {
                return new ExploitDetails(this.user, simpleName, "Double.MAX_VALUE position", false, true, "set \"position-checks.allow-invalid-movement-v2\" to true");
            }
            if (a4 >= 2.147483647E9 || b5 > 2.147483647E9 || (c2 >= 2.147483647E9 && !Settings.IMP.POSITION_CHECKS.ALLOW_INVALID_MOVEMENT_V1)) {
                return new ExploitDetails(this.user, simpleName, "Integer.MAX_VALUE position", false, true, "set \"position-checks.allow-invalid-movement-v1\" to true");
            }
            final float a5 = packetPlayInFlying.a(0.0f);
            final float b6 = packetPlayInFlying.b(0.0f);
            if (a5 == Float.NEGATIVE_INFINITY || b6 == Float.NEGATIVE_INFINITY || a5 >= Float.MAX_VALUE || b6 >= Float.MAX_VALUE) {
                return new ExploitDetails(this.user, simpleName, "invalid float position", false, true);
            }
            if (Settings.IMP.POSITION_CHECKS.CHECK_YAW_CRASHER && (a5 < -80000.0f || b6 < -80000.0f || a5 > 80000.0f || b6 > 80000.0f || a5 == 9.223372E18 || b6 == 9.223372E18 || a5 == 9.223372E18f || b6 == 9.223372E18f)) {
                return new ExploitDetails(this.user, simpleName, String.valueOf(new StringBuilder().append("Invalid yaw/pitch v3 (yaw: ").append(a5).append(", pitch: ").append(b6).append(")")), false, false, "set position-checks.check-yaw-crasher to false in settings.yml report to staff on mc-protection.eu/discord");
            }
            if (Math.abs(b5) > 1.0E9) {
                return new ExploitDetails(this.user, simpleName, String.valueOf(new StringBuilder().append("Invalid Y coords (").append(b5).append(")")), false, false, "report to staff on mc-protection.eu/discord");
            }
        }
        else if (o instanceof PacketPlayInHeldItemSlot) {
            final int b7 = ((PacketPlayInHeldItemSlot)o).b();
            if (b7 >= 36 || b7 < 0) {
            ...
            final double distance = new Location(player.getWorld(), (double)blockPosition.getX(), (double)blockPosition.getY(), (double)blockPosition.getZ()).distance(player.getLocation());
            if (max_DISTANCE_BETWEEN_PLAYER_AND_BLOCK_POSITION > 0 && distance > max_DISTANCE_BETWEEN_PLAYER_AND_BLOCK_POSITION) {
                return new ExploitDetails(this.user, simpleName, String.valueOf(new StringBuilder().append("invalid UseItem blockPosition (current: ").append(distance).append(", item: ").append((player.getItemInHand() == null) ? "null" : player.getItemInHand().getType().name()).append(")")), false, true, "set position-checks.max-distance-between-player-and-block-position to -1");
            }
        }
    ...
    private LimitablePacket hasReachedLimit(final String s) {
        if (!PacketDecoder_1_17_R1.LIMITATION.isLimitable(s)) {
            return null;
        }
        final LimitablePacket limit = PacketDecoder_1_17_R1.LIMITATION.getLimit(s);
    ...
    public PacketDecoder_1_17_R1(final PacketInjector packetInjector, final User user) {
                ...
                if (compound2.hasKey("textures")) {
                    ...
                    for (int l = 0; l < list2.size(); ++l) {
                        ...
                        if (compound3.hasKey("Value")) {
                            ...
                            final Matcher matcher = PacketDecoder_1_17_R1.URL_MATCHER.matcher(lowerCase2);
                            while (matcher.find()) {
