...
public class PacketDecoder_1_16_R3 extends PacketDecoder
{
        ...
        if (o.getClass().getSimpleName().equals("PacketPlayInWindowClick")) {
            ...
            try {
                ...
                if (openInventory != null) {
                    ...
                    if (b >= n && PacketDecoder_1_16_R3.LIMITATION != null) {
                    ...
                    if (f != null) {
                        ...
                        if (bukkitCopy.getType() == Material.CHEST || bukkitCopy.getType() == Material.HOPPER) {
                            ...
                            if (f.getTag() != null && f.getTag().toString().contains("www.wurstclient.net")) {
                ...
                if (string.equals("MC|BEdit") || string.equals("MC|BSign") || string.equals("minecraft:bedit") || string.equals("minecraft:bsign")) {
                    ...
                    try {
                        ...
                        if (n2 != null) {
                            ...
                            final ExploitDetails checkNbtTags3 = this.checkNbtTags(simpleName, n2);
            ...
            if (Settings.IMP.POSITION_CHECKS.CHECK_CHUNK_CRASHER) {
                final double n3 = Math.max(player.getLocation().getX(), a) - Math.min(player.getLocation().getX(), a);
                if (n3 >= 10.0 && n3 % 1.0 == 0.0) {
                    ...
                    if (n3 > 1000.0) {
                        return new ExploitDetails(this.user, simpleName, String.valueOf(new StringBuilder().append("Chunk crasher (v2, invalid x coords, diff: ").append(n3).append(")")), false, false, "set position-checks.check-chunk-crasher to false in settings.yml");
                    }
                    if (this.hasReachedLimit("PacketPlayInPosition_Invalid_v1") != null) {
                        return new ExploitDetails(this.user, simpleName, String.valueOf(new StringBuilder().append("Chunk crasher (v1, invalid x coords, diff: ").append(n3).append(")")), false, false, "set position-checks.check-chunk-crasher to false in settings.yml or increase PacketPlayInPosition_Invalid_v1.limit in config.yml");
                    }
                }
                final double n4 = Math.max(player.getLocation().getZ(), c) - Math.min(player.getLocation().getZ(), c);
                if (n4 >= 10.0 && n4 % 1.0 == 0.0) {
                    SpigotGuardLogger.log(Level.INFO, String.valueOf(new StringBuilder().append("Received invalid location packet from: ").append(player.getName()).append(" (").append(player.getLocation().getX()).append(",").append(player.getLocation().getY()).append(",").append(player.getLocation().getZ()).append(" ||| Packet: ").append(a).append(",").append(b2).append(",").append(c).append("), difference: ").append(n4)), new Object[0]);
                    if (n4 > 1000.0) {
                        return new ExploitDetails(this.user, simpleName, String.valueOf(new StringBuilder().append("Chunk crasher (v2, invalid z coords, diff: ").append(n4).append(")")), false, false, "set position-checks.check-chunk-crasher to false in settings.yml");
                    }
                    if (this.hasReachedLimit("PacketPlayInPosition_Invalid_v1") != null) {
                        return new ExploitDetails(this.user, simpleName, String.valueOf(new StringBuilder().append("Chunk crasher (v1, invalid z coords, diff: ").append(n4).append(")")), false, false, "set position-checks.check-chunk-crasher to false in settings.yml or increase PacketPlayInPosition_Invalid_v1.limit in config.yml");
                    }
                }
            }
            if (Settings.IMP.POSITION_CHECKS.CHECK_FLY_CRASHER && b2 == player.getLocation().getY() + 0.1) {
                final double n5 = Math.max(player.getLocation().getY(), b2) - Math.min(player.getLocation().getY(), b2);
                ...
                if (this.hasReachedLimit("PacketPlayInPosition_Invalid_v2") != null) {
                    return new ExploitDetails(this.user, simpleName, String.valueOf(new StringBuilder().append("Fly crasher (v1, invalid y coords, diff: ").append(n5).append(")")), false, false, "set position-checks.check-fly-crasher to false in settings.yml or increase PacketPlayInPosition_Invalid_v2.limit in config.yml");
                }
            }
    ...
    public PacketDecoder_1_16_R3(final PacketInjector packetInjector, final User user) {
                ...
                if (compound2.hasKey("textures")) {
                    ...
                    for (int l = 0; l < list2.size(); ++l) {
                        ...
                        if (compound3.hasKey("Value")) {
                            ...
                            final Matcher matcher = PacketDecoder_1_16_R3.URL_MATCHER.matcher(lowerCase2);
                            while (matcher.find()) {
    ...
    private LimitablePacket hasReachedLimit(final String s) {
        if (!PacketDecoder_1_16_R3.LIMITATION.isLimitable(s)) {
            return null;
        }
        final LimitablePacket limit = PacketDecoder_1_16_R3.LIMITATION.getLimit(s);
